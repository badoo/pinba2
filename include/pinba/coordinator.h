...
#define COORDINATOR_REQ__DEL_REPORT 1

struct coordinator_request_t
	: public nmsg_message_ex_t<coordinator_request_t>
{
	int type;
};
...
struct coordinator_response_t
	: public nmsg_message_ex_t<coordinator_response_t>
{
	int            status;
	meow::error_t  err;

	coordinator_response_t(int s)
		: status(s)
		, err()
	{
	}

	coordinator_response_t(int s, std::string const& err_string)
		: status(s)
		, err(err_string)
	{
	}
